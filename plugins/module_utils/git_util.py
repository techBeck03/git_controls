...
git_argument_spec = dict(
    git_token=dict(
        fallback=(env_fallback, ['GIT_TOKEN']), type='str', no_log=True),
    git_username=dict(fallback=(env_fallback, ['GIT_USERNAME']), type='str'),
    git_password=dict(
        fallback=(env_fallback, ['GIT_PASSWORD']), type='str', no_log=True),
    repo=dict(fallback=(env_fallback, [
              'GIT_REPO']), type='str', required=True),
...
FileReturn = TypedDict(
    'FileReturn', {'status': str, 'changed_files': List[str]})
...
class GitModule():
    ...
    def __init__(self, module):
        ...
        if not git_token and not git_username:
            self.module.fail_json(
                msg='One of `git_token` or `git_username` is required')
        if git_token and git_username:
            self.module.fail_json(
                msg='Only one of `git_token` or `git_username` is allowed')
        if git_username and not git_password:
            self.module.fail_json(
                msg='`git_password` is required with `git_username`')
    ...
    def addFiles(self, files: List[str]) -> FileReturn:
        ...
        if repo.bare:
            self.module.fail_json(
                msg='Working directory does not appear to contain a git repo')
    ...
    def removeFiles(self, files: List[str]) -> FileReturn:
        ...
        if repo.bare:
            self.module.fail_json(
                msg='Working directory does not appear to contain a git repo')
        valid_files = [
            f"{workingDir}/{file}" for file in files if Path(f"{workingDir}/{file}").exists()]
    ...
    def cloneRepo(self, auto_generate_parent: bool, pull: bool) -> Text:
        ...
        """
        workingDir = Path(self.module.params.get("working_dir")) if not auto_generate_parent else Path(
            f'{self.module.params.get("working_dir")}/{str(int(time() * 1000))}')
        if workingDir.exists():
            repo = Repo(workingDir)
            if repo.bare or repo.remotes.origin.url != self.repo_url:
                self.module.fail_json(
                    msg="Specified working directory already exists and doesn't contain the desired git repo")
            if pull:
        ...
        workingDir.mkdir()
        Repo.clone_from(self.repo_url, workingDir,
